        ...
        >
          {testimonials.map((t, i) => (
            <motion.blockquote
              key={i}
              variants={fadeIn}
              className="relative rounded-[--radius-xl] border border-border bg-surface-1 p-8 flex flex-col gap-4 overflow-hidden"
            >
              {/* Decorative Playfair italic opening quote */}
              <span
                className="absolute top-2 left-4 font-display italic text-7xl leading-none text-primary-light select-none pointer-events-none"
                aria-hidden="true"
              >
                &ldquo;
              </span>
